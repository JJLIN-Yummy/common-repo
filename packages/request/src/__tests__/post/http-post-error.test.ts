...
describe('POST请求错误测试', () => {
  ...
  // });
  createHttpErrorTest(() => request().response(), '测试POST 401错误请求', 401);
  createHttpErrorTest(() => request().response(), '测试POST 403错误请求', 403);
  createHttpErrorTest(() => request().response(), '测试POST 404错误请求', 404);
  createHttpErrorTest(() => request().response(), '测试POST 500错误请求', 500);
});
